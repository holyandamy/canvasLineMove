 ...
 */
// console.log('获取的div是'+document.getElementById("c").offsetHeight);
console.log('获取的浏览器高是：'+$(window).height());
var starBoff = true;
$(".loadBox").css("height", $(window).height());//设置.loadBox的高是浏览器的高
$(document).on("mousemove",//当鼠标在文档可视区移动时，阻止默认事件
function(a) {
    a.preventDefault();
    return false
});
function judge() {//判断浏览器类型
    var a = {};
    var b = navigator.userAgent.toLowerCase();
    var d; 
	(d = b.match(/msie ([\d.]+)/)) ? a.ie = d[1] : (d = b.match(/firefox\/([\d.]+)/)) ? a.firefox = d[1] : (d = b.match(/chrome\/([\d.]+)/)) ? a.chrome = d[1] : (d = b.match(/opera.([\d.]+)/)) ? a.opera = d[1] : (d = b.match(/version\/([\d.]+).*safari/)) ? a.safari = d[1] : 0; (d = b.match(/rv:([\d.]+)/)) ? a.ie11 = d[1] : "";
    if (a.firefox) {
...
var loadAni = {
    init: function() {//初始化
        ...
        b.c = document.getElementById("c");console.log("canvas"+this.c.height);
        b.c.height = "400";
       b.c.width = "600";
        b.C1 = c.getContext("2d");
        b.C1.strokeStyle = "pink";//#dd657c
        b.offsetY = -3;
...
var currentLoadSource = [ "img/loadBg.png","img/loadPen.png", "img/loadRoute.png"];
